class_name CanMoveThePlayerDownWebRTC extends MdsTestScene
...
func _ready() -> void:
	test_label = "Can move the player down - WebRTC"
	test_case()

func _enter_tree() -> void:
	var server_multiplayer = SceneMultiplayer.new()
	get_tree().set_multiplayer(server_multiplayer, ^"/root/CanMoveThePlayerDownWebRTC/PeerA")
	var client_multiplayer = SceneMultiplayer.new()
	get_tree().set_multiplayer(client_multiplayer, ^"/root/CanMoveThePlayerDownWebRTC/PeerB")
...
func test_case():
	...
	assert(peer_a_player_a.position.z > 5.0, "[PeerA] PlayerA.position.z should be positive")
	...
	assert(peer_b_player_a.position.z > 5.0, "[PeerB] PlayerA.position.z should be positive")
	assert(peer_b_player_b.position.z == 10.0, "[PeerB] PlayerB.position.z should be 10")
	await end_of_physics_frame
	
	# Move up with PeerB
	await start_of_physics_frame
	send_input("down_b", true)
	await end_of_physics_frame
	
	# Assert PeerB local changes
	assert(peer_b_player_b.position.z > 10.0, "[PeerB] PlayerB.position.z should be positive")
	assert(peer_a_player_b.position.z == 10.0, "[PeerA] PlayerB.position.z should be 10")
	
	# Assert PeerA side after sync
	await start_of_physics_frame
	await peer_a_player_b.synchronizer.synchronized
	assert(peer_a_player_b.position.z > 10.0, "[PeerA] PlayerB.position.z should be positive")
	await end_of_physics_frame
